            ...
            if (copies > 1)
            {
                ...
                for (int i = 0; i < copies; i++)
                {
                    ...
                    foreach (var q in query) { 
                        ...
                        cvpf.processDate = String.Format("{0:d}", q.processDate.ToString("dd/MM/yyyy"));//DateTime.Now.ToString("dd/MM/yyyy"));
                        cvpf.rank = q.rank;
                        cvpf.form = q.form;
                        cvpf.OiC = q.OiC;
                ...
                var vpf = (from pa in db.VarietyProcessFlows.AsEnumerable().Where(pa => pa.processId == id)
                           select new CustomVarietyProcessFlow
                           {
                               ...
                               OiC = pa.user.FirstName + " " + pa.user.LastName,
                               form = pa.form.name,
                               processDate = String.Format("{0:d}", pa.processDate.ToString("dd/MM/yyyy")),//DateTime.Now.ToString("dd/MM/yyyy")),//pa.processDate),//.HasValue ? pa.processDate.Value.ToString("dd/MM/yyyy") : String.Empty,
                               rank = pa.rank.name,
                               barcode = pa.barcode,
            ...
            if (copies > 1)
            {
                ...
                for (int i = 0; i < copies; i++)
                {
                    ...
                    foreach (var q in vpf)
                    {
                        ...
                        cvpf.barcode = q.barcode;//Regex.Match(q.barcode, @"\d+").Value;
                        cvpf.processId = q.processId;
                        cvpf.processDate = (q.processDate.HasValue)? q.processDate.ToString() : null; //DateTime.Now.ToString("dd/MM/yyyy");//
                        customVPF.Add(cvpf);
                    }
                }
            }
